					...
					if (Object.prototype.hasOwnProperty.call(p, 'formato') && Object.prototype.hasOwnProperty.call(p, 'dados')) {
						if (Object.prototype.hasOwnProperty.call(p.dados, 'executar')) {
									...
									p.dados.input.forEach(key => {
										if (key.length === 3) {
											...
											if (dataType.base) {
												...
												_throwThis('DB', 500, `${key[1]} data type (query input) was not found in method, please correct or notify an administrator...`);
											}
										} else {
											if (key.length === 2) {
												r.input(key[0], key[1]);
											} else {
												_throwThis('DB', 500, `Format { ${key} } is invalid (query input), it requires two or three keys depending on the calling model...`);
											}
										}
									});
								}
									...
									p.dados.output.forEach(key => {
										if (key.length === 2) {
											...
											if (dataType.base) {
												...
												_throwThis('DB', 500, `${key[1]} data type (query output) was not found in method, please correct or notify an administrator...`);
											}
										} else {
											_throwThis('DB', 500, `Format { ${key} } is invalid (query output), it requires two keys...`);
										}
									});
								}
							};
							...
							switch (p.formato) {
								...
								default: {
									_throwThis('DB', 500, 'Format not correctly defined in JSON parameters, it only works with the following values: 1 (local queries) or 2 (stored procedure)...');
								}
							}
						} else {
							_throwThis('DB', 500, 'Execute not correctly defined in JSON parameters, check your code...');
						}
					} else {
						_throwThis('DB', 500, 'Format and/or data not correctly defined in the JSON parameters, check your code...');
					}
				};
